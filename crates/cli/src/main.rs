...
use mise_core::types::{EffortClass, Slug};
use mise_store::pages::{RecipeDoc, StateDoc};
...
fn run(store: &mut Store, command: Cmd, root: &Path, now: Zoned) -> Result<()> {
    ...
    match command {
        ...
        Cmd::Queue { cmd: Some(QueueCmd::Add { title, recipe, reason, id, someday }) } => {
            let input = json!({
                "title": title, "recipe": recipe, "reason": reason,
                "id": id, "someday": someday,
            });
            println!("{}", tool_edit(store, &now, "queue_add", input)?);
            Ok(())
        }
        Cmd::Queue { cmd: Some(QueueCmd::Remove { id, someday }) } => {
            let input = json!({ "id": id, "someday": someday });
            println!("{}", tool_edit(store, &now, "queue_remove", input)?);
            Ok(())
        }
        Cmd::Chat { message, page, model, photos } => {
            run_chat(store, message, page, model, photos, now)
        }
        Cmd::Recipe { cmd } => run_recipe(store, cmd, &now),
        Cmd::Equipment { cmd } => run_equipment(store, cmd, &now),
        Cmd::Pantry { cmd } => run_pantry(store, cmd, &now),
        Cmd::Fridge { cmd } => run_fridge(store, cmd, &now),
        Cmd::Log { cmd } => run_log(store, cmd, &now),
...
fn run_recipe(store: &mut Store, cmd: RecipeCmd, now: &Zoned) -> Result<()> {
    match cmd {
        RecipeCmd::Add {
            slug: s,
            title,
            ...
            let body = body.map(|p| read_body(&p)).transpose()?;
            let input = json!({
                "slug": s, "title": title, "servings": servings,
                "effort": effort.to_string(), "tags": parse_tags(&tags)?,
                "equipment": equipment, "lead_minutes": lead_minutes,
                "lead_step": lead_step, "body": body,
            });
            println!("{}", tool_edit(store, now, "recipe_add", input)?);
            Ok(())
        }
        RecipeCmd::Ingredient { slug: s, text, link } => {
            // Appending one line is recipe_edit's full-list replace over the
            // current ingredients — read, append, submit — so the shared tool
            // owns validation and the CLI stays a marshaller.
            let s = slug(&s)?;
            let recipe: RecipeDoc = store.get(&DocId::Recipe(s.clone()))?;
            let mut ingredients: Vec<_> = recipe
                .ingredients
                .iter()
                .map(|i| json!({ "text": i.text, "pantry": i.pantry.as_ref().map(|p| p.to_string()) }))
                .collect();
            ingredients.push(json!({ "text": text, "pantry": link }));
            let input = json!({ "slug": s.to_string(), "ingredients": ingredients });
            println!("{}", tool_edit(store, now, "recipe_edit", input)?);
            Ok(())
        }
        RecipeCmd::Body { slug: s, file } => {
            let body = read_body(&file)?;
            let input = json!({ "slug": s, "body": body });
            println!("{}", tool_edit(store, now, "recipe_edit", input)?);
            Ok(())
        }
    }
...
fn run_equipment(store: &mut Store, cmd: EquipmentCmd, now: &Zoned) -> Result<()> {
    match cmd {
        EquipmentCmd::Add { item, note, location } => {
            let input = json!({ "item": item, "note": note, "location": location });
            println!("{}", tool_edit(store, now, "equipment_set", input)?);
            Ok(())
        }
        EquipmentCmd::Remove { item, location } => {
            let input = json!({ "item": item, "location": location });
            println!("{}", tool_edit(store, now, "equipment_remove", input)?);
            Ok(())
        }
    }
}

fn run_pantry(store: &mut Store, cmd: PantryCmd, now: &Zoned) -> Result<()> {
    match cmd {
        PantryCmd::Set { item, name, presence, tier, bought, note, location } => {
            let input = json!({
                "item": item, "name": name, "presence": presence, "tier": tier,
                "bought": bought, "note": note, "location": location,
            });
            println!("{}", tool_edit(store, now, "pantry_set", input)?);
            Ok(())
        }
        PantryCmd::Remove { item, location } => {
            let input = json!({ "item": item, "location": location });
            println!("{}", tool_edit(store, now, "pantry_remove", input)?);
            Ok(())
        }
    }
}

fn run_fridge(store: &mut Store, cmd: FridgeCmd, now: &Zoned) -> Result<()> {
    match cmd {
        FridgeCmd::Add { dish, servings, date, freezer, location } => {
            let input = json!({
                "dish": dish, "servings": servings, "date": date,
                "freezer": freezer, "location": location,
            });
            println!("{}", tool_edit(store, now, "fridge_add", input)?);
            Ok(())
        }
        FridgeCmd::Remove { id, freezer, location } => {
            let input = json!({ "id": id, "freezer": freezer, "location": location });
            println!("{}", tool_edit(store, now, "fridge_remove", input)?);
            Ok(())
        }
    }
}

fn run_log(store: &mut Store, cmd: LogCmd, now: &Zoned) -> Result<()> {
    match cmd {
        LogCmd::Add { title, recipe, kind, servings, verdict, date, tags, location } => {
            // The tool inherits the recipe's tags and servings default and
            // bounds servings; the CLI only turns k=v pairs into the map shape.
            let input = json!({
                "title": title, "recipe": recipe, "kind": kind, "servings": servings,
                "verdict": verdict, "date": date, "tags": parse_tags(&tags)?, "location": location,
            });
            println!("{}", tool_edit(store, now, "log_append", input)?);
            Ok(())
        }
...
/// Every CLI mutation goes through the assistant tool layer — the one internal
/// mutation API, which the HTTP surface (`/api/edit`) already uses. The CLI's
/// job is to marshal clap args into the tool's JSON input; validation, id
/// minting, tier checks, location resolution and upsert semantics live in
/// `tools.rs`, once. A `Fail::User` (model-recoverable: bad input, unknown
/// slug) becomes a `bail!`; a store failure propagates. One export per edit,
/// as the HTTP consumer does.
fn tool_edit(store: &mut Store, now: &Zoned, tool: &str, input: serde_json::Value) -> Result<String> {
    use mise_assistant::tools::{self, ToolCtx};
    use mise_assistant::turn::ToolCall;

    let ctx = ToolCtx { now: now.clone(), provenance: "cli".into() };
    let call = ToolCall { id: "cli".into(), name: tool.into(), input };
    let outcome = tools::execute(store, &ctx, &call).map_err(anyhow::Error::new)?;
    if outcome.is_error {
        bail!("{}", outcome.content);
    }
    store.export(&format!("cli: {}", outcome.content))?;
    Ok(outcome.content)
}
